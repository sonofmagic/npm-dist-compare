"use strict";
const picker = require("./picker-BE4o6Ps6.cjs");
...
const time = require("./time-DdYNue6x.cjs");
    ...
    handleYearTableClick(event) {
      ...
      if (target.tagName === "A") {
        ...
        if (this.selectionMode === "years") {
          ...
          const idx = util.arrayFindIndex(value, (date) => date.getFullYear() === Number(year));
...
const YearTable = __component__$5.exports;
...
const getMonthTimestamp = function(time2) {
...
const removeFromArray$1 = function(arr, pred) {
  ...
  methods: {
    ...
    handleMonthTableClick(event) {
      ...
      if (this.selectionMode === "range") {
        ...
        const value = this.value || [];
        const year = this.date.getFullYear();
        const newValue = util.arrayFindIndex(value, (date) => date.getFullYear() === year && date.getMonth() === month) >= 0 ? removeFromArray$1(value, (date) => date.getTime() === newDate.getTime()) : [...value, newDate];
        this.$emit("pick", newValue);
      } else {
    ...
    rows() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          const index = i * 4 + j;
          const time2 = new Date(this.date.getFullYear(), index).getTime();
          cell.inRange = time2 >= getMonthTimestamp(this.minDate) && time2 <= getMonthTimestamp(this.maxDate);
          cell.start = this.minDate && time2 === getMonthTimestamp(this.minDate);
          cell.end = this.maxDate && time2 === getMonthTimestamp(this.maxDate);
          const isToday = time2 === now;
          if (isToday) {
            cell.type = "today";
          }
          cell.text = index;
          let cellDate = new Date(time2);
...
const getDateTimestamp = function(time2) {
  if (typeof time2 === "number" || typeof time2 === "string") {
    ...
    rows() {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (this.showWeekNumber) {
          if (!row[0]) {
            row[0] = { type: "week", text: dateUtil.getWeekNumber(dateUtil.nextDate(startDate, i * 7 + 1)) };
          }
        }
        for (let j = 0; j < 7; j++) {
          ...
          cell.inRange = time2 >= getDateTimestamp(this.minDate) && time2 <= getDateTimestamp(this.maxDate);
          cell.start = this.minDate && time2 === getDateTimestamp(this.minDate);
          cell.end = this.maxDate && time2 === getDateTimestamp(this.maxDate);
          const isToday = time2 === now;
          if (isToday) {
            cell.type = "today";
          }
...
const DateTable = __component__$3.exports;
const _sfc_main$2 = {
  mixins: [Locale],
...
const _sfc_main$1 = {
  ...
  computed: {
    ...
    minVisibleDate() {
      ...
      if (this.minDate) return dateUtil.formatDate(this.minDate, this.dateFormat);
      return "";
    },
    maxVisibleDate() {
      if (this.dateUserInput.max !== null) return this.dateUserInput.max;
      if (this.maxDate || this.minDate) return dateUtil.formatDate(this.maxDate || this.minDate, this.dateFormat);
      return "";
    },
    minVisibleTime() {
      if (this.timeUserInput.min !== null) return this.timeUserInput.min;
      if (this.minDate) return dateUtil.formatDate(this.minDate, this.timeFormat);
      return "";
    },
    maxVisibleTime() {
      if (this.timeUserInput.max !== null) return this.timeUserInput.max;
      if (this.maxDate || this.minDate) return dateUtil.formatDate(this.maxDate || this.minDate, this.timeFormat);
      return "";
    },
    timeFormat() {
      if (this.format) {
...
const DateRangePanel = __component__$1.exports;
