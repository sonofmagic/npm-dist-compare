...
import { isDate, getDayCountOfYear, range, nextDate, getDayCountOfMonth, getWeekNumber, prevDate, getFirstDayOfMonth, getStartDateOfMonth, clearTime, extractDateFormat, extractTimeFormat, formatDate, timeWithinRange, parseDate, modifyTime, modifyDate, modifyWithTimeString, changeYearMonthAndClampDate, nextYear, prevYear, nextMonth, prevMonth, clearMilliseconds } from "element-ui/lib/utils/date-util";
import Clickoutside from "element-ui/lib/utils/clickoutside";
import Locale from "element-ui/lib/mixins/locale";
import ElInput from "element-ui/lib/input";
import ElButton from "element-ui/lib/button";
import { T as TimePanel } from "./time-BYZdsOND.js";
  ...
  methods: {
    ...
    handleMonthTableClick(event) {
      ...
      if (this.selectionMode === "range") {
        ...
        const value = this.value || [];
        const year = this.date.getFullYear();
        const newValue = arrayFindIndex(value, (date) => date.getFullYear() === year && date.getMonth() === month) >= 0 ? removeFromArray$1(value, (date) => date.getTime() === newDate.getTime()) : [...value, newDate];
        this.$emit("pick", newValue);
      } else {
...
const _sfc_main$1 = {
  ...
  methods: {
    ...
    handleDateInput(value, type) {
      ...
      if (parsedValue) {
        ...
        if (type === "min") {
          this.minDate = modifyDate(this.minDate || /* @__PURE__ */ new Date(), parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate());
          this.leftDate = new Date(parsedValue);
          if (!this.unlinkPanels) {
            this.rightDate = nextMonth(this.leftDate);
          }
        } else {
          this.maxDate = modifyDate(this.maxDate || /* @__PURE__ */ new Date(), parsedValue.getFullYear(), parsedValue.getMonth(), parsedValue.getDate());
  ...
  components: { TimePicker: TimePanel, DateTable, ElInput, ElButton }
};
