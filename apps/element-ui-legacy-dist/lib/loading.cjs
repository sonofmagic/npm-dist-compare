...
const afterLeave = require("element-ui/lib/utils/after-leave");
    ...
    if (binding.value) {
      Vue2.nextTick(() => {
        if (binding.modifiers.fullscreen) {
          ...
          el.maskStyle.zIndex = Popup.PopupManager.nextZIndex();
          dom.addClass(el.mask, "is-fullscreen");
          insertDom(document.body, el, binding);
        } else {
          dom.removeClass(el.mask, "is-fullscreen");
          if (binding.modifiers.body) {
            el.originalPosition = dom.getStyle(document.body, "position");
            ["top", "left"].forEach((property) => {
              const scroll = property === "top" ? "scrollTop" : "scrollLeft";
              el.maskStyle[property] = el.getBoundingClientRect()[property] + document.body[scroll] + document.documentElement[scroll] - parseInt(dom.getStyle(document.body, `margin-${property}`), 10) + "px";
            });
...
const addStyle = (options, parent, instance) => {
  ...
  if (options.fullscreen) {
    ...
    maskStyle.zIndex = Popup.PopupManager.nextZIndex();
...
const Loading = {
  install(Vue2) {
    ...
    Vue2.prototype.$loading = Loading$1;
  },
