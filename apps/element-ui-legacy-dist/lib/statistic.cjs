"use strict";
const lodash = require("element-ui/lib/utils/lodash");
...
const _sfc_main = {
  ...
  methods: {
    ...
    formatTimeStr: function(time) {
      ...
      let formatText = lodash.reduce(
        timeUnits,
        (con, item) => {
          const name = item[0];
          return con.replace(new RegExp(`${name}+`, "g"), (match) => {
            let sum = lodash.chain(time).divide(item[1]).floor(0).value();
            time -= lodash.multiply(sum, item[1]);
            return lodash.padStart(String(sum), String(match).length, 0);
          });
        },
        format
      );
