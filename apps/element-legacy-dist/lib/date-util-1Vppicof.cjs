...
function getI18nSettings() {
  return {
    dayNamesShort: weeks.map((week) => locale.t(`el.datepicker.weeks.${week}`)),
    dayNames: weeks.map((week) => locale.t(`el.datepicker.weeks.${week}`)),
    monthNamesShort: months.map((month) => locale.t(`el.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => locale.t(`el.datepicker.month${index + 1}`)),
    amPm: ["am", "pm"]
  };
}
