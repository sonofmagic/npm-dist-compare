...
const _sfc_main = {
  ...
  methods: {
    ...
    handleButtonEnter(arrow) {
      ...
      this.items.forEach((item, index2) => {
        if (arrow === this.itemInStage(item, index2)) {
          item.hover = true;
        }
      });
    },
    ...
    resetItemPosition(oldIndex) {
      this.items.forEach((item, index2) => {
        item.translateItem(index2, this.activeIndex, oldIndex);
      });
    },
    playSlides() {
    ...
    setActiveItem(index2) {
      if (typeof index2 === "string") {
        const filteredItems = this.items.filter((item) => item.name === index2);
  ...
  created() {
    this.throttledArrowClick = index.throttle(300, (index2) => {
      this.setActiveItem(index2);
    }, {
      // TODO
    });
    this.throttledIndicatorHover = index.throttle(300, (index2) => {
      this.handleIndicatorHover(index2);
    });
  },
  mounted() {
    this.updateItems();
    this.$nextTick(() => {
      resizeEvent.addResizeListener(this.$el, this.resetItemPosition);
  ...
  beforeDestroy() {
    if (this.$el) resizeEvent.removeResizeListener(this.$el, this.resetItemPosition);
    this.pauseTimer();
  }
};
var _sfc_render = function render() {
  ...
  return _c("div", { class: _vm.carouselClasses, on: { "mouseenter": function($event) {
    ...
    $event.stopPropagation();
    return _vm.throttledArrowClick(_vm.activeIndex + 1);
  } } }, [_c("i", { staticClass: "el-icon-arrow-right" })])]) : _vm._e(), _vm._t("default")], 2), _vm.indicatorPosition !== "none" ? _c("ul", { class: _vm.indicatorsClasses }, _vm._l(_vm.items, function(item, index2) {
    return _c("li", { key: index2, class: [
      "el-carousel__indicator",
      "el-carousel__indicator--" + _vm.direction,
      { "is-active": index2 === _vm.activeIndex }
    ], on: { "mouseenter": function($event) {
      return _vm.throttledIndicatorHover(index2);
    }, "click": function($event) {
      $event.stopPropagation();
      return _vm.handleIndicatorClick(index2);
...
_Carousel.install = function install(Vue) {
