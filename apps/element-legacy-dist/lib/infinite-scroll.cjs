...
const getScrollOptions = (el, vm) => {
  if (!types.isHtmlElement(el)) return {};
  return entries(attributes).reduce((map, [key, option]) => {
    const { type, default: defaultValue } = option;
    let value = el.getAttribute(`infinite-scroll-${key}`);
    value = types.isUndefined(vm[value]) ? value : vm[value];
    switch (type) {
      case Number:
        value = Number(value);
        ...
        break;
      case Boolean:
        value = types.isDefined(value) ? value === "false" ? false : Boolean(value) : defaultValue;
        break;
      default:
        value = type(value);
    }
...
const handleScroll = function(cb) {
  ...
  if (shouldTrigger && types.isFunction(cb)) {
    cb.call(vm);
  } else if (observer) {
    observer.disconnect();
...
const InfiniteScroll = {
  ...
  inserted(el, binding, vnode) {
    ...
    const onScroll = index.debounce(delay, handleScroll.bind(el, cb));
...
module.exports = _InfiniteScroll;
