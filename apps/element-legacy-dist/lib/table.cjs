...
const util = require("./util-4-As88B0.cjs");
const util$1 = require("./util-KJN0EjuU.cjs");
const scrollbarWidth = require("./scrollbar-width-Fu3YgEXO.cjs");
...
const Watcher = Vue.extend({
  ...
  methods: {
    ...
    clearFilter(columnKeys) {
      ...
      let panels = {};
      if (tableHeader) panels = merge.merge(panels, tableHeader.filterPanels);
      if (fixedTableHeader) panels = merge.merge(panels, fixedTableHeader.filterPanels);
      if (rightFixedTableHeader) panels = merge.merge(panels, rightFixedTableHeader.filterPanels);
...
class TableLayout {
  constructor(options) {
    ...
    this.gutterWidth = scrollbarWidth.scrollbarWidth();
...
const _sfc_main = {
  name: "ElTable",
  mixins: [locale.Locale, migrating.Migrating],
  directives: {
    Mousewheel
  },
  props: {
