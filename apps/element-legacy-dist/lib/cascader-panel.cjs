...
const ElRadio = require("element-ui/lib/radio");
...
const _sfc_main$2 = {
  ...
  methods: {
    ...
    renderCheckbox(h) {
      ...
      return h("el-checkbox", helper._mergeJSXProps2([{
        "attrs": {
          "value": node.checked,
    ...
    renderRadio(h) {
      ...
      if (util.isEqual(value, checkedValue)) {
        value = checkedValue;
      }
      return h("el-radio", {
        "attrs": {
  ...
  render(h) {
    ...
    return h("li", helper._mergeJSXProps2([{
      "attrs": {
        "role": "menuitem",
        "id": nodeId,
...
const _sfc_main$1 = {
  ...
  methods: {
    ...
    renderNodeList(h) {
      ...
      const nodes = this.nodes.map((node, index) => {
        ...
        return h("cascader-node", helper._mergeJSXProps2([{
          "key": node.uid,
          "attrs": {
            "node": node,
  ...
  render(h) {
    ...
    return h("el-scrollbar", helper._mergeJSXProps2([{
      "attrs": {
        "tag": "ul",
        "role": "menu",
        "id": menuId,
...
class Node {
  ...
  get isLeaf() {
    ...
    if (lazy) {
      const isLeaf2 = shared.isDef(data[leafKey]) ? data[leafKey] : loaded ? !children.length : false;
  ...
  isSameNode(checkedValue) {
    ...
    return this.config.multiple && Array.isArray(checkedValue) ? checkedValue.some((val) => util.isEqual(val, value)) : util.isEqual(checkedValue, value);
  }
...
class Store {
  ...
  initNodes(data) {
    data = util.coerceTruthyValueToArray(data);
  ...
  appendNodes(nodeDataList, parentNode) {
    nodeDataList = util.coerceTruthyValueToArray(nodeDataList);
  ...
  getNodeByValue(value) {
    const nodes = this.getFlattedNodes(false, !this.config.lazy).filter((node) => util.valueEquals(node.path, value) || node.value === value);
    return nodes && nodes.length ? nodes[0] : null;
  }
}
const { keys: KeyCode } = ariaUtils.AriaUtils;
const DefaultProps = {
  expandTrigger: "click",
  // or hover
  multiple: false,
...
const _sfc_main = {
  ...
  computed: {
    config() {
      return merge.merge({ ...DefaultProps }, this.props || {});
    },
    multiple() {
  ...
  watch: {
    ...
    checkedValue(val) {
      if (!util.isEqual(val, this.value)) {
        this.checkStrictly && this.calculateCheckedNodePaths();
        this.$emit("input", val);
  ...
  methods: {
    initStore() {
      ...
      if (config.lazy && util.isEmpty(options)) {
        this.lazyLoad();
      } else {
    ...
    calculateCheckedNodePaths() {
      ...
      const checkedValues = multiple ? util.coerceTruthyValueToArray(checkedValue) : [checkedValue];
    ...
    handleExpand(node, silent) {
      ...
      if (!silent) {
        ...
        if (!util.valueEquals(pathValues, activePathValues)) {
    ...
    scrollIntoView() {
      ...
      menus.forEach((menu) => {
        ...
        if (menuElement) {
          ...
          scrollIntoView.scrollIntoView(container, activeNode);
        }
      });
    },
    getNodeByValue(val) {
...
_CascaderPanel.install = function install(Vue) {
  Vue.component(_CascaderPanel.name, _CascaderPanel);
};
module.exports = _CascaderPanel;
